...
def multi_metapath_graph_edgevalue_premeta(hg,metapaths):
    ...
    for metapath in metapaths:
        ...
        for etype in metapath:
            temp = hg.adj(etype=etype, scipy_fmt='csr')
            adj = adj * temp
            # adj = prune(adj)
        # adj = prune(adj)
        # print("conducting",metapath)
        # with open('DBLPneighbors.txt'+str(metapath), 'w+') as file:
        #     for (x,y) in zip(adj.nonzero()[0],adj.nonzero()[1]):
        #         print(x," ",y,file=file)
        res_adj = res_adj + adj
...
#         val_mask, test_mask, meta_paths, dl = load_data('DBLP2', feat_type=0)
# print(hg)
# #
#
