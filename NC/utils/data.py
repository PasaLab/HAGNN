...
def load_data(prefix='DBLP'):
    ...
    if prefix == 'PubMed' or prefix == 'ACM2' or prefix == 'DBLP2':
        print("dag")
        dl = data_loader4dag('../../data/' + prefix)
    else:
        print("normal")
    ...
    if prefix == 'PubMed':
        labels = np.zeros((dl.nodes['count'][1], dl.labels_train['num_classes']), dtype=int)
    else:
        labels = np.zeros((dl.nodes['count'][0], dl.labels_train['num_classes']), dtype=int)
    val_ratio = 0.2
    train_idx = np.nonzero(dl.labels_train['mask'])[0]
    if prefix == 'PubMed':
        train_idx = train_idx - 13561
...
def load_data4homo(prefix='DBLP'):
